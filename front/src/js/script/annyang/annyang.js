...
(function (undefined) {
  ...
  root.annyang = {
    ...
    customCallCommands: function customCallCommands(data){
      var commandText;
      for (var i = 0; i<data.length; i++) {
        // the text recognized
        commandText = data[i].trim();
        if (debugState) {
        ...
        for (var j = 0, l = commandsList.length; j < l; j++) {
          ...
          if (result) {
            ...
            invokeCallbacks(callbacks.resultMatch, commandText, currentCommand.originalPhrase, data);
            return true;
          }
        }
      }
      invokeCallbacks(callbacks.resultNoMatch, data);
      return false;
    },
